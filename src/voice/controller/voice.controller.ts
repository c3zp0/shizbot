...
export class VoiceController {
    ...
    async getTodayTopVoiceUsers(ctx: CustomContext) {
        ...
        const top = await this._voiceService.getTopVoicesLengthByChat(ctx.chat.id, { start, end });
        if (top.size === 0) {
    ...
    async getTopVoiceUsers(ctx: CustomContext) {
        ...
        const topTotal = await this._voiceService.getTopVoicesLengthByChat(ctx.chat.id);
        if (topTotal.size === 0) {
            await ctx.api.sendMessage(ctx.chat.id, 'Еще нет ни одного зарегистрированного голосовного сообщения');
    ...
    async handleVoice(ctx: Filter<CustomContext, ':voice' | ':video_note'>) {
        ...
        await this._voiceService.create(duration, fileId, fileUniqueId, fileSize, ctx.user, chatId);
        if (ctx.isOutDatedMessage) {
            throw new Error('Message is out dated');
        }
        const totalVoices = await this._voiceService.getUserVoicesCount(ctx.user, ctx.chat.id);
        const now = new Date();
        const start = new Date(`${now.getFullYear()}-${now.getMonth() + 1}-${now.getDate()}`);
        const end = new Date(start);
        end.setHours(23);
        ...
        const todaysVoices = await this._voiceService.getUserVoicesCount(ctx.user, ctx.chat.id, { start, end });
    ...
    async ownVoicesLength(ctx: CustomContext) {
        ...
        const totalUserVoicesLength = await this._voiceService.getUsersVoicesLength(ctx.user, ctx.chat.id);
        const todayUserVoicesLength = await this._voiceService.getUsersVoicesLength(ctx.user, ctx.chat.id, {
            start,
            end,
        });

        const totalChatVoicesLength = await this._voiceService.getChatTotalVoicesDuration(ctx.chat.id);
